...
import { createDish, getDishes, getDishesWithIngredients, getDishById } from "../services/dishes";
...
const dishesController = {
    ...
    getDishesWithIngredients: async (req: Request, res: Response) => {
        try {
            const page = parseInt(req.query.page as string) || 1;
            const limit = parseInt(req.query.limit as string) || 10;
            
            const dishes = await getDishesWithIngredients(page, limit);
            res.status(200).json(dishes);
        } catch (error) {
            res.status(500).json({ message: "Error fetching dishes with ingredients" });
        }
    },
